...
func (t *Transport) dial(ctx context.Context, addr net.Addr, host string, tlsConf *tls.Config, conf *Config, use0RTT bool) (EarlyConnection, error) {
	...
	return dial(ctx, newSendConn(t.conn, addr, packetInfo{}, utils.DefaultLogger, host), t.connIDGenerator, t.handlerMap, tlsConf, conf, onClose, use0RTT)
}
...
func (t *Transport) listen(conn rawConn) {
	...
	for {
		...
		// See https://github.com/nofish24/quic-go/issues/1737 for details.
